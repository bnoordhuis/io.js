...
struct write_req {
  write_req(inspector_socket_t* inspector, const char* data, size_t size)
      : inspector(inspector)
...
static void write_request_cleanup(uv_write_t* req, int status) {
  write_req* wr = node::ContainerOf(&write_req::req, req);
  delete wr;
}
...
static int write_to_client(inspector_socket_t* inspector,
  ...
  write_req* wr = new write_req(inspector, msg, len);
