{
  'targets': [
...

    {
      ...
      'type': 'static_library',
      'toolsets': ['host', 'target'],
      'dependencies': ['icu_config', 'icuuc_base', 'icudata'],
      'direct_dependent_settings': {
    ...
    {
      'target_name': 'icuuc_base',
      'type': 'static_library',
      'toolsets': ['host', 'target'],
      'defines': ['U_COMMON_IMPLEMENTATION=1'],
      'dependencies': ['icu_config'],
      'direct_dependent_settings': {
        'include_dirs': ['source/common'],
      },
      'include_dirs': ['source/common'],
...

    {
      ...
      'dependencies': ['icu_config', 'icuuc_base'],
      'direct_dependent_settings': {
        'include_dirs': ['source/i18n'],
      },
      'include_dirs': ['source/i18n'],
      'sources': [
    ...
    # TODO(bnoordhuis) Break out byteswap logic in icupkg into something that
    # can be used by mkicudata.cc so we don't need to build ICU for both host
    # and target when the target is big-endian.
    {
      'target_name': 'icudata',
      'type': 'static_library',
      'toolsets': ['host', 'target'],
      'conditions': [
        ['target_arch in "mips ppc ppc64 s390 s390x"', {
          'dependencies': ['icupkg#host', 'mkicudata#host'],
          'actions': [
            {
              'action_name': 'icudata_bswap',
              'action': ['<@(_inputs)', '<@(_outputs)', '-tb'],
              'inputs': ['<(PRODUCT_DIR)/icupkg', '<(icu_gyp_dir)/icudata.dat'],
              'outputs': ['<(SHARED_INTERMEDIATE_DIR)/icudata.dat'],
            },
            # TODO(bnoordhuis) .incbin converted file if toolchain supports it.
            {
              'action_name': 'icudata_bin2c',
              'action': ['<@(_inputs)', '<@(_outputs)', 'bin2c', '<(OS)'],
              'inputs': [
                '<(PRODUCT_DIR)/mkicudata',
                '<(SHARED_INTERMEDIATE_DIR)/icudata.dat',
              ],
              'outputs': ['<(SHARED_INTERMEDIATE_DIR)/icudata.cpp'],
              'process_outputs_as_sources': 1,
            },
          ],
        }, 'OS=="win"', {
          'dependencies': ['mkicudata#host'],
          'actions': [
            {
              'action_name': 'icudata_bin2c',
              'action': ['<@(_inputs)', '<@(_outputs)', 'bin2c', '<(OS)'],
          ...
          'actions': [
            {
              'action_name': 'icudata_incbin',
              'action': ['<@(_inputs)', '<@(_outputs)', 'incbin', '<(OS)'],
              'inputs': [
                '<(PRODUCT_DIR)/mkicudata',
                '<(icu_gyp_dir)/icudata.dat',
              ],
              'outputs': ['<(SHARED_INTERMEDIATE_DIR)/icudata.s'],
              'process_outputs_as_sources': 1,
            },
          ],
        }],
    ...
    {
      'target_name': 'mkicudata',
      'type': 'executable',
      'toolsets': ['host'],
      'include_dirs': ['source/common'],
      'sources': ['mkicudata.cc'],
    },

    {
      'target_name': 'icupkg',
      'type': 'executable',
      'dependencies': ['icu_config', 'icuuc_base', 'icui18n'],
      'toolsets': ['host'],
      'defines': ['U_TOOLUTIL_IMPLEMENTATION=1'],
      'include_dirs': ['source/tools/toolutil'],
      'sources': [
