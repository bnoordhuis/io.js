...
(function(process, binding) {
    ...
    Object.defineProperty(global, 'console', {
      ...
      get: function() {
        if (!console) {
          ...
          installInspectorConsoleIfNeeded(console, inspectorConsole);
        }
        return console;
      }
    });
  ...
  function installInspectorConsoleIfNeeded(console, inspectorConsole) {
    if (!inspectorConsole)
      return;
    const config = {};
    for (const key of Object.keys(console)) {
      ...
      // the original stack.
      console[key] = binding.inspectorWrapConsoleCall(inspectorConsole[key],
                                                      console[key],
                                                      config);
    }
  ...
  function setupProcessICUVersions() {
    if (!binding.hasIntl) return;
    process.versions.cldr = binding.cldrVersion;
    process.versions.icu = binding.icuVersion;
    process.versions.tz = binding.tzdataVersion;
    process.versions.unicode = binding.unicodeVersion;
  }
  ...
  function run(entryFunction) {
    if (binding.debugWaitConnect && isDebugBreak()) {
  ...
  if (process.execArgv.some((arg) => /^--expose[-_]internals$/.test(arg)))
    NativeModule.isInternal = (id) => false;
  else
    NativeModule.isInternal = (id) => id.startsWith('internal/');
